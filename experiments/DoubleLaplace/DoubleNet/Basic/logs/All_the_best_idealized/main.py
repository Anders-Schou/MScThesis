...
class PINN01(DoubleLaplacePINN):
    ...
    def loss_terms(self,
        ...
        if update_key == 2:
            ...
            self.loss_names = ["phi", "psi"] + [f"rect{i}" for i, _ in enumerate(loss_rect)] + [f"circ{i}" for i, _ in enumerate(loss_circ)] + [f"data{i}" for i, _ in enumerate(loss_data)]
        ...
        idealized_rect_true = {"xx0": jnp.zeros(inputs["rect"][0].shape[0]),
                               "xy0": jnp.zeros(inputs["rect"][0].shape[0]),
                               "yy1": jnp.zeros(inputs["rect"][1].shape[0]) + 10,
                               "xy1": jnp.zeros(inputs["rect"][1].shape[0]),
                               "xx2": jnp.zeros(inputs["rect"][2].shape[0]),
                               "xy2": jnp.zeros(inputs["rect"][2].shape[0]),
                               "yy3": jnp.zeros(inputs["rect"][3].shape[0]) + 10,
                               "xy3": jnp.zeros(inputs["rect"][3].shape[0])}
        
        loss_coll = self.loss_coll(params, inputs["coll"], true_val=true_val.get("coll"))
        loss_rect = self.loss_rect(params, inputs["rect"], true_val=idealized_rect_true)
        loss_circ = self.loss_circ(params, inputs["circ"], true_val=true_val.get("circ"))
...
if __name__ == "__main__":
    ...
    timer(pinn.train)(update_key=None)
