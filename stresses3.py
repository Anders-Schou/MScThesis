...
class PPINN:
    ...
    def __init__(self, net: MLP, run_settings: dict, loss_fn = None):
        ...
        self.optimizer = run_settings["optimizer"](learning_rate=run_settings["learning_rate"])
    ...
    def rect_bc2(self, params, x):
        out0 = self.hessian(params, x[0])
        out1 = self.hessian(params, x[1])
        out2 = self.hessian(params, x[2])
        out3 = self.hessian(params, x[3])
        hl = ((out0[:, 0, 0] - 10)**2).mean() + ((out0[:, 0, 1])**2).mean()
        hu = ((out1[:, 0, 0] - 10)**2).mean() + ((out1[:, 0, 1])**2).mean()
        vr = ((out2[:, 1, 0])**2).mean() + ((out2[:, 1, 1])**2).mean()
        vl = ((out3[:, 1, 0])**2).mean() + ((out3[:, 1, 1])**2).mean()
        return hl + hu + vr + vl

    def circle_bc2(self, params, x: jnp.ndarray, u_bc: jnp.ndarray) -> float:
        out = self.hessian(params, x)
        rtheta_stress = jax.vmap(cart2polar_tensor, in_axes=(0, 0))(out, x)
    ...
    def loss(self,
        ...
        return self.circle_bc2(params, xc, ucpp) + self.pde(params, xp) + self.rect_bc2(params, xr)
...
if __name__ == "__main__":
    ...
    u_stress = jnp.full((num_bc, 1), TENSION)
    u_zero = jnp.zeros((num_bc, 1))
    ...
    if do_result_plots:
        ...
        Zpptrue = [Zhesstrue.reshape(-1, 4)[:, i].reshape(X.shape)*(xy2r(X, Y) >= RADIUS) for i in range(4)]
