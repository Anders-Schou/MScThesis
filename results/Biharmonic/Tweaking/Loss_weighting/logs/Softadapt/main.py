...
class PINN01(BiharmonicPINN):
    ...
    def train(self, update_key = None, epochs: int | None = None, new_init: bool = False, start_epoch: int = 0) -> None:
        """
...
if __name__ == "__main__":
    ...
    #     all_losses = jnp.load(f)
    
    # del_indices = jnp.arange(pinn.train_settings.checkpoint_every // pinn.logging.log_every, pinn.train_settings.checkpoint_every // pinn.logging.log_every -int(((((all_losses.shape[0])* pinn.logging.log_every) % pinn.train_settings.checkpoint_every) / pinn.logging.log_every)), -1)
    # pinn.all_losses = jnp.delete(all_losses, del_indices, 0)
    
    # timer(pinn.train)(start_epoch=pinn.checkpoint_epoch)
    # timer(pinn.write_model)()
    # # timer(pinn.load_model)()
    # timer(pinn.plot_results)()
